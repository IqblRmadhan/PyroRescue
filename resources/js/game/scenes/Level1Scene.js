...
export default class Level1Scene extends Phaser.Scene {
    ...
    pinStatusToCamera() {
        const statusObjects = this.children.list.filter((child) => child.depth >= 20);

        for (const object of statusObjects) {
            object.setScrollFactor(0);
            object.setPosition(object.x / cameraZoom, object.y / cameraZoom);
            object.setScale(object.scaleX / cameraZoom, object.scaleY / cameraZoom);
        }
    }

    drawStatus() {
        const textStyle = { fontFamily: 'monospace', fontSize: '19px', color: '#fff4cd' };
        this.add.rectangle(622, 36, 322, 46, 0x102c2a, 0.95).setStrokeStyle(2, 0xcca365).setDepth(20);
        this.add.image(490, 34, 'icons', 'fire').setDisplaySize(23, 30).setDepth(21);
        this.fireText = this.add.text(513, 27, '', { ...textStyle, fontSize: '15px' }).setDepth(21);

        this.add.rectangle(738, 94, 110, 44, 0x102c2a, 0.95).setStrokeStyle(2, 0xcca365).setDepth(20);
        this.add.image(704, 93, 'icons', 'water').setDisplaySize(22, 29).setDepth(21);
        this.waterText = this.add.text(722, 83, '', textStyle).setDepth(21);
    }

    setWater(amount) {
        this.water = amount;
        this.waterText.setText(`x ${amount}`);
        this.waterText.setFontSize(amount > 9999 ? '11px' : '19px');
    }
    ...
    updateFireLevel() {
        if (this.currentFireLevel === 0) {
            ...
            this.fireText.setText('Api berhasil dipadamkan');
            return;
        }
        ...
        this.fireText.setText(
            `Api ${fireSizes[this.currentFireLevel]} - ${this.currentFireLevel} air`,
        );
    }
